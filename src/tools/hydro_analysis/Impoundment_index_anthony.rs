...
pub struct ImpoundmentIndexAnthony {
    name: String,
    description: String,
    toolbox: String,
...
impl ImpoundmentIndexAnthony {
    pub fn new() -> ImpoundmentIndexAnthony {
        // public constructor
        let name = "ImpoundmentIndexAnthony".to_string();
...
impl WhiteboxTool for ImpoundmentIndexAnthony {
    ...
    fn run<'a>(
        ...
        for i in 0..args.len() {
            ...
            } else if flag_val == "-streamextraction" {
                stream_extraction = if keyval {
                    vec[1].to_string().parse::<f64>().unwrap()
                } else {
                    args[i + 1].to_string().parse::<f64>().unwrap()
                };
        ...
        */
        ...
        let mut stream_threshold: f64; //ADDED
        let mut num_upslope: f64;
        let mut vol: f64;
        let mut output_impoundment = Raster::initialize_using_file(&output_file, &input);
        output_impoundment.reinitialize_values(0.0);
        while !stack.is_empty() {
            ...
            if dir >= 0 {
                ...
                stream_threshold = crest_elev.get_value(row, col); //ADDED
                num_upslope = 0f64;
                vol = 0f64;
                ...
                for up_z in upslope_elevs[row as usize][col as usize].clone() {
                    if up_z < cutoff_z {
                        ...
                        if up_z < threshold {
                            if stream_threshold >= stream_extraction {
                                num_upslope += 1f64;
                                vol += threshold - up_z  
                            }
                        }
                ...
                if out_type == 0 {
                    // area
                    output_impoundment.increment(row_n, col_n, num_upslope * grid_area);
                } else {
                    // volume
                    output_impoundment.increment(row_n, col_n, vol);
                }
        ...
        output_impoundment.configs.palette = "spectrum.plt".to_string();
        output_impoundment.add_metadata_entry(format!(
            "Created by whitebox_tools\' {} tool",
            self.get_tool_name()
        ));
        output_impoundment.add_metadata_entry(format!("Input file: {}", input_file));
        output_impoundment.add_metadata_entry(format!("Dam length: {}", dam_length));
        if out_type == 0 {
            output_impoundment.add_metadata_entry(format!("Out type: flooded area"));
        } else {
            output_impoundment.add_metadata_entry(format!("Out type: reservoir volume"));
        }
        output_impoundment.add_metadata_entry(
        ...
        let _ = match output_impoundment.write() {
            Ok(_) => if verbose {
