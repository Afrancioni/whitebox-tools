...
pub struct FeaturePreservingDenoiseAnthony {
    name: String,
    description: String,
    toolbox: String,
...
impl FeaturePreservingDenoiseAnthony {
    pub fn new() -> FeaturePreservingDenoiseAnthony { // public constructor
        let name = "FeaturePreservingDenoiseAnthony".to_string();
        ...
        parameters.push(ToolParameter{
            name: "Streams File".to_owned(), 
            flags: vec!["-s".to_owned(), "--streaminput".to_owned()], 
        ...
        parameters.push(ToolParameter {
            name: "Stream Extraction Threshold".to_owned(),
            flags: vec!["--streamextraction".to_owned()],
            description: "Stream Extraction Threshold.".to_owned(),
        ...
        FeaturePreservingDenoiseAnthony { 
            name: name, 
...
impl WhiteboxTool for FeaturePreservingDenoiseAnthony {
    ...
    fn run<'a>(&self, args: Vec<String>, working_directory: &'a str, verbose: bool) -> Result<(), Error> {
        ...
        let mut streams_input_file = String::new();
        let mut output_file = String::new();
        let mut filter_size = 11usize;
        let mut stream_extraction = 10f64;
        ...
        for i in 0..args.len() {
            ...
            } else if vec[0].to_lowercase() == "-s" || vec[0].to_lowercase() == "--streaminput" {
                if keyval {
                    streams_input_file = vec[1].to_string();
                } else {
                    streams_input_file = args[i+1].to_string();
                }
            ...
            } else if vec[0].to_lowercase() == "-streamextraction" {
                if keyval {
                    stream_extraction = vec[1].to_string().parse::<f64>().unwrap()
                } else {
                    stream_extraction = args[i + 1].to_string().parse::<f64>().unwrap()
                };
        ...
        let streams_input = Arc::new(Raster::new(&streams_input_file, "r")?);
        ...
        let t1 = time::now();
        println!("{}", format!("Calculating normal vectors: {}", t1 - start).replace("PT", ""));
        ...
        for tid in 0..num_procs {
            let input = input.clone();
            let streams_input = streams_input.clone();
            let nv = nv.clone();
            let tx = tx.clone();
            thread::spawn(move || {
                ...
                let mut w: f64;
                let mut stream: f64;
                let stream_value = stream_extraction; //threshold value
                let mut sum_w: f64;
                for row in (0..rows).filter(|r| r % num_procs == tid) {
                    let mut data = vec![Normal { a: 0f64, b: 0f64, c: 0f64 }; columns as usize];
                    for col in 0..columns {
                        z = input.get_value(row, col);
                        if z != nodata {
                            stream = streams_input.get_value(row, col);
                            sum_w = 0f64;
                            a = 0f64;
                            b = 0f64;
                            c = 0f64;
                            for n in 0..num_pixels_in_filter {
                                ...
                                if input.get_value(yn, xn) != nodata {
                                    ...
                                    //stream_value = streams_input.get_value(yn, xn);
                                    if diff > threshold {
                                        if stream <= stream_value { //IF THE STREAMS IS LESS THAN 10... THEN SET THE THRESHOLD IN THE OUTPUT OF THE IMPOUNDMENT 
                ...
                for col in 0..columns {
                    ...
                    if z != nodata {
                        ...
                        for n in 0..8 {
                            ...
                            if zn != nodata {
                                diff = nv_smooth.get_value(row, col).angle_between(nv_smooth.get_value(yn, xn));
                                if diff > threshold {
                                    if stream <= stream_value {
                                        w = (diff - threshold)*(diff - threshold);
                                        sum_w += w;
                                        z += -(nv_smooth.get_value(yn, xn).a * x[n] + nv_smooth.get_value(yn, xn).b * y[n] - nv_smooth.get_value(yn, xn).c * zn) / nv_smooth.get_value(yn, xn).c * w;
                                    }
                                    //w = (diff - threshold)*(diff - threshold);
                                    //sum_w += w;
                                    //z += -(nv_smooth.get_value(yn, xn).a * x[n] + nv_smooth.get_value(yn, xn).b * y[n] - nv_smooth.get_value(yn, xn).c * zn) / nv_smooth.get_value(yn, xn).c * w;
                        ...
                        if sum_w > 0f64 { // this is a division-by-zero safeguard and must be in place.
        ...
        output.add_metadata_entry(format!("Streams file: {}", streams_input_file));
        ...
        output.add_metadata_entry(format!("Stream Extraction Threshold {}", stream_extraction));
